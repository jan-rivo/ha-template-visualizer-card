...
| # | Item | Status | Notes |
|---|------|--------|-------|
| A1 | Rewrite README (current features, config-options table, corrected supported-subset incl. `{% if %}`/`{% set %}`, fresh light+dark screenshots; keep "Important limitation") | pending | `hacs.json` has `render_readme: true` — README is the HACS storefront |
