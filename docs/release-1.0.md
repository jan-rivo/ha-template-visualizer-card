...
| B4 | Enable Dependabot (npm + GitHub Actions) | done | `.github/dependabot.yml` (weekly Monday, npm minor/patch grouped, majors + actions separate, `chore` prefix, assigned to jan-rivo). Activates on merge to `main` — first PRs expected the following Monday. Policy 2026-09-07: merge dep PRs to `main` (rebuild `dist/` on the PR branch first so CI goes green); no version bump or release per dep update — deps ride along until the next real release; majors get a harness screenshot re-check |
| B5 | Add bug-report issue template (HA version + template text) | done | `.github/ISSUE_TEMPLATE/bug_report.yml` (YAML issue form: HA version + card version required, template text required, what-happened required, screenshots/console optional; auto-labels `bug`, title prefix `[bug]: `) |
| B6 | Branch protection on `main` (require CI) | done | Rule on `main`: require status checks (CI + HACS validation) + branches up to date. No PR requirement (solo dev); direct pushes unaffected, PR merges blocked on red CI |
| B7 | Fill in GitHub About section/topics (`home-assistant`, `lovelace`, `hacs`) | done | Description + topics set on the repo homepage |
...
| 2026-09-07 | B5 done: bug-report issue template added. |
